...
int main() {
    printf("Periodic Task Test:\n");
    
    // Set period to 10 ticks
    setperiod(10);
    ...
    for (int i = 0; i < 3; i++) {
        ...
        printf("Iteration %d: Current tick = %d\n", i, current_uptime);
        
        // Wait until next period
        wait_until_next_period();
        
        int after_wait_uptime = uptime();
        printf("  After wait: tick = %d, elapsed = %d\n", 
               after_wait_uptime, after_wait_uptime - current_uptime);
    }
